...
// Create a map of operation IDs to tags from the swagger spec
...
Object.keys(paths).forEach(path => {
  Object.keys(paths[path]).forEach(method => {
    ...
    if (operation.operationId) {
      ...
      opIdToTags.set(operation.operationId, tags);
    }
  });
});

console.log(`  - Tag mappings created: ${opIdToTags.size}`);
...
markdown += '| Operation ID | HTTP Method | Endpoint | Description | Tags | On Developer Portal | Source |\n';
markdown += '|--------------|-------------|----------|-------------|------|---------------------|--------|\n';

allApis.forEach(api => {
  const opId = api.operationId || '';
  const method = api.method || 'N/A';
  const path = (api.path || '').substring(0, 50) || 'N/A';
  const desc = (api.description || '').substring(0, 60).replace(/\|/g, '/');
  const tags = (api.tags || []).join(', ').substring(0, 30) || 'N/A';
  const onPortal = api.onDeveloperPortal ? '✅ Yes' : '❌ No';
  const source = api.source || '';

  markdown += `| ${opId} | ${method} | ${path} | ${desc} | ${tags} | ${onPortal} | ${source} |\n`;
});
...
Object.keys(tagMap).sort().forEach(tag => {
  ...
  tagMap[tag].slice(0, 10).forEach(opId => {
    markdown += `- ${opId}\n`;
  });
  if (tagMap[tag].length > 10) {
    markdown += `- ... and ${tagMap[tag].length - 10} more\n`;
  }
  markdown += '\n';
});
...
console.log('\nFiles created:');
console.log('  - all-public-apis.json (structured data with tags)');
console.log('  - all-public-apis-report.md (markdown report with tags)');
